    ...
    $('#send-btn').click(function() {
        $.ajax({
            ...
            success: function(data, status) {
                ...
                $('#main').append('<dl class="paper a1"><dt>' 
                    + '<span class="username">'
                    + data.username 
                    + '</span>' 
                    + '<span class="num">' 
                    ...
                    + '</span>' 
                    + '<a href="" class="close"></a></dd></dl>');
                var $newp = $('.paper:last');
                $newp.css({
                    left: parseInt(Math.random() * (FW - $newp.width())) + 'px',
                    top: parseInt(Math.random() * (FH - $newp.height())) + 'px'
                });
                drag($newp, $('dt', $newp));
...
function drag(obj, element) {
    ...
    $(document).mousemove(function(event) {
        ...
        var left, top; //计算定位宽、高 防止跳出视口
        //OX - DX < 0防止拉到左侧边缘 OY - DY < 0防止拉倒上侧边缘
        //OX - DX > DW - OW 防止拉倒右侧边缘超出视口
        //OY - DY > DH - OH ? DH - OH 同上
